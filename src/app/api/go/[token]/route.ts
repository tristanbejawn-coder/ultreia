...
export async function GET(_: Request, ctx: { params: Promise<{ token: string }> }) {
  ...
  const bundle = await dbSelect<MessageRow>(`messages?walk_id=eq.${auth.walk.id}&deleted_at=is.null&select=id,from_name,body,written_at,delivered_at&order=written_at.desc&limit=100`)
